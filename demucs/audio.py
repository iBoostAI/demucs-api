...
def save_audio(wav: torch.Tensor,
    ...
    if suffix == ".mp3":
        ...
        if suffix == ".wav" and as_float:
            # Float32 WAV: keep as float
            samples = wav
        elif suffix == ".wav" and bits_per_sample == 16:
            # 16-bit PCM WAV: convert to int16
            samples = i16_pcm(wav).float() / (2**15 - 1)
        elif suffix == ".wav" and bits_per_sample == 24:
            # 24-bit PCM WAV: clamp to [-1, 1] for encoder
            samples = wav.clamp(-1, 1)
        else:
            # FLAC: pass as float, encoder handles bit depth
            samples = wav.clamp(-1, 1)
        encoder = AudioEncoder(samples=samples, sample_rate=samplerate)
